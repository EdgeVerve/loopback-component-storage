...
exports.upload = function(provider, req, res, options, cb) {
  ...
  form.handlePart = function(part) {
    ...
    if (maxFileSize) {
      ...
      part.on('data', function(buffer) {
        if(!detectedMimeType) {
          detectedMimeType = fileType(buffer);
          if (detectedMimeType && detectedMimeType.mime && options.allowedContentTypes) {
            var allowedContentTypes;
            if ('function' === typeof options.allowedContentTypes) {
              allowedContentTypes = options.allowedContentTypes(file, req, res);
            } else {
            ...
            if (Array.isArray(allowedContentTypes) && allowedContentTypes.length !== 0) {
              if (allowedContentTypes.indexOf(detectedMimeType.mime) === -1) {
                var err = new Error('mimetype unsupported');
                err.statusCode = 400;
                self._error(err);
                return;
              }
